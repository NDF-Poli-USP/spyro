...
def truncated_bnd_points(xdom, y0, a, b, n, perimeter, lmax):

    # Boundary points: min 16 or 24
    num_bnd_pts = int(max(np.ceil(perimeter / lmax), 16)) - 1

    # Generate the hyperellipse boundary points
    pnt_bef_cut = 0
    pnt_aft_cut = 0
    while pnt_bef_cut % 2 == 0 or pnt_aft_cut % 2 == 0 \
            or pnt_bef_cut < 3 or pnt_aft_cut < 3:
        ...
        # Filter hyperellipse points based on the y0
        filt_bnd_pts = np.array([point for point in bnd_pts
                                 if point[1] <= y0])

        # Identify truncation index
        ini_cut = max(np.where(bnd_pts[:, 1] > y0)[0][0] - 1, 0)

        # Modify points to match with the truncation plane
        r0 = np.asin((y0 / b)**(n / 2))
        x0 = a * np.cos(r0)**(2/n)
        filt_bnd_pts[ini_cut] = np.array([x0, y0])
        filt_bnd_pts[ini_cut + 1] = np.array([-x0, y0])

        # Insert new points to create a rectangular cut
        new_pnts = np.array([[xdom, y0], [xdom, -y0],
                             [-xdom, -y0], [-xdom, y0]])
        filt_bnd_pts = np.insert(filt_bnd_pts, ini_cut + 1, new_pnts, axis=0)
        end_cut = ini_cut + 5

        # Points before and after truncation
        pnt_bef_cut = len(filt_bnd_pts[:ini_cut + 1])
        pnt_aft_cut = len(filt_bnd_pts[end_cut:])
...
def create_composite_mesh(Lx, Ly, pad, n, lmin, lmax):
    ...
    for idp in range(ini_cut, end_cut):
        p1 = geo.PointData()[2][idp]
        p2 = geo.PointData()[2][idp + 1]
        # print(p1, p2)

        if idp == ini_cut or idp == end_cut - 1:
            curves.append(["line", p1, p2, lcut])
        else:
            curves.append(["line", p1, p2, lmin])

    for idp in range(end_cut, num_bnd_pts - 1, 2):
        p1 = geo.PointData()[2][idp]
        p2 = geo.PointData()[2][idp + 1]
        p3 = geo.PointData()[2][idp + 2]
        curves.append(["spline3", p1, p2, p3, lcut])
