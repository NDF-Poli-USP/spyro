...
for degree in degrees:
    ...
    if dimension == 2 and quadrilateral == False:
        num_elements = (2*n**2)
        new_alpha = 1./np.sqrt(dof/ (num_elements) )
    elif dimension == 2 and quadrilateral == True:
        num_elements = (n**2)
        new_alpha = 1./np.sqrt(dof/ (num_elements) )
    elif dimension == 3 and quadrilateral == False:
        num_elements = (6*n**3)
        new_alpha = 1./np.cbrt(dof/ (num_elements) )
    elif dimension == 3 and quadrilateral == True:
        num_elements = (n**3)
        new_alpha = 1./np.cbrt(dof/ (num_elements) )
