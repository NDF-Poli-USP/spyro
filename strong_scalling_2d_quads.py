...
model["opts"] = {
    ...
    "quadrature": "spectral",  # Equi or KMV
...
M = 2.64
edge_length = 1.496/(5.0*M)
nz = int(Lz/edge_length)
nx = int(Lx/edge_length)
mesh = fire.RectangleMesh(nx, nz, Lz, Lx, quadrilateral=True, comm=comm.ensemble_comm)

mesh.coordinates.dat.data[:,0]*= -1
mesh.coordinates.dat.data[:,1]-= model["BCs"]["lx"]
element = fire.FiniteElement("CG", mesh.ufl_cell(), degree=model["opts"]["degree"], variant="spectral")
