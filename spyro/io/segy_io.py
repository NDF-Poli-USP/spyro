...
def export_scalar_field(function, grid_spacing, output_filename, comm=None):
    ...
    ----------
    function : firedrake.Function
        Scalar field to export.
    grid_spacing : float
        Spacing of output grid points used when sampling ``function``.
    output_filename : str
    ...
    -------
    None
        Writes output files to disk. For ``.png`` output, an intermediate
        SEG-Y file is created alongside the PNG.

    Notes
    -----
    - ``.segy`` and ``.sgy`` outputs are written directly from the sampled
      field.
    - ``.png`` output is generated by first writing a temporary SEG-Y file
      with the same basename and then converting it to PNG.
    """
    # if output_filename.lower().endswith((".png", ".segy", ".sgy")):
    V = function.function_space()
    if output_filename.lower().endswith((".segy", ".sgy")):
        create_segy(function, V, grid_spacing, output_filename)
        ...
        raise ValueError(f"Invalid extension, please use {valid_extensions}")
