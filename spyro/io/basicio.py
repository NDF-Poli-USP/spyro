...
def ensemble_propagator(func):
    ...
    def wrapper(*args, **kwargs):
        requested_shot_ids = kwargs.get("shot_ids")
        legacy_source_nums = kwargs.get("source_nums")
        if requested_shot_ids is not None or legacy_source_nums is not None:
        ...
        if args[0].parallelism_type != "spatial" or args[0].number_of_sources == 1:
            ...
            for propagation_id, shot_ids_in_propagation in enumerate(shot_ids_per_propagation_list):
                if is_owner(_comm, propagation_id):
                    func(
                        *args, **dict(kwargs, shot_ids=shot_ids_in_propagation)
                    )
            ...
            for snum in range(num):
                ...
                u, u_r = func(*args, **dict(kwargs, shot_ids=[snum]))
