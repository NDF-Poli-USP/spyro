...
class Model_parameters(Read_boundary_layer, VelocityModelFileIO, Read_time_axis):
    ...
    def __init__(self, dictionary=None, comm=None):
        ...
        if self.analysis == "transient":
            ...
            self.save_forward_solution = self.input_dictionary["time_axis"].get(
                "save_forward_solution", True
            )
        ...
        if value is not None:
            for receiver in value:
                ...
                _check_point_in_domain(
                    receiver_points, mesh_lengths, self.mesh_parameters.negative_z
                )
    ...
    def set_mesh(self, user_mesh=None, input_mesh_parameters={}):
        ...
        if self.mesh_parameters.automatic_mesh:
            automatic_mesh = meshing.AutomaticMesh(mesh_parameters=self.mesh_parameters)
            self.user_mesh = (
...
def _check_point_in_domain(point_coordinates, input_mesh_lengths, negative_z):
    ...
    for i, (coord, length) in enumerate(zip(point_coordinates, mesh_lengths)):
        coo_str = f"Coordinate {coord} in dimension {i} is outside the domain "
