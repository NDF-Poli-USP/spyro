...
class MeshFile(Mesh):
    ...
    def get_mesh(self, mesh_definition: MeshDefinition):
        ...
        return fire.Mesh(
            ...
            } if method in [FiniteElementMethod.CG_TRIANGLE, FiniteElementMethod.MASS_LUMPED_TRIANGLE] else None,
        )
...
class MeshDefinition:
    ...
    def assert_point_in_domain(self, point_coordinates):
        ...
        for i, (coord, length) in enumerate(zip(point_coordinates, self.domain_dimensions)):
            error_message = f"Coordinate {coord} in dimension {i} is outside the domain "
