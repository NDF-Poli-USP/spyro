...
def compute_functional(
    wave_object,
    misfit,
    evaluation_mode=FunctionalEvaluationMode.AFTER_SOLVE,
    step=None,
    nsteps=None,
    functional_form=FunctionalType.L2Norm,
):
    r"""Compute the functional value for the given misfit at receiver locations.
    ...
    if evaluation_mode == FunctionalEvaluationMode.PER_TIMESTEP:
        ...
        if wave_object.use_vertex_only_mesh:
            return assemble(0.5 * wave_object.dt * weight * inner(misfit, misfit) * dx)
