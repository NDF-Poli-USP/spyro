...
def get_real_shot_record(wave):
    ...
    if isinstance(real_shot_record, np.ndarray):
        if (
            ...
            and real_shot_record.shape[1] == wave.number_of_receivers
        ):
            return real_shot_record
        if (
            real_shot_record.ndim >= 3
            and real_shot_record.shape[2] == wave.number_of_receivers
        ):
    ...
    if isinstance(real_shot_record, (list, tuple)):
        if (
            ...
            if (
                ...
                and source_record.shape[1] == wave.number_of_receivers
            ):
