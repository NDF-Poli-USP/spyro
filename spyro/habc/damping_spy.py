...
def regFreq(factLen, nexp, a, b):
    ...
    '''
    factRec = (pi / 2) * (1 / a**2 + 1 / b**2)**0.5  # Eval factor rectangle
...
def calcFact2D(TipLay, Lx, Ly, pml, nexp, CamComp=False):
    ...
    if TipLay == 'REC':
        ...
        '''
        M01 = float(broyden1(F, 0, f_tol=1e-14,
...
def testPosDamp(mesh_Fin, Lx, Ly, pml, ref):
    ...
    ref[testc] = ((refamx[testc] - 0.5*Lx)**2 +
                  (refamy[testc] - 0.5*Ly)**2)**0.5
...
def CRminQua(d, kCR, xCR=None, typ='QUA', alpha=1,
             p=None, psi=None, a=None, F_L=None):
    '''
    ...
    if typ == 'QUA':  # Minimum coefficient reflection
        ...
        p2 = p*(1 + 1/8*(psiMdf*a/F_L)**2)  # For fundamental mode m=1
        Z1 = Zi(p1, alpha)
        Z2 = Zi(p2, alpha)
        num = (1-Z1)*np.sin(p1) + (alpha*Z2 - 1)*np.sin(alpha*p2)/alpha
        den = (1-Z1)*np.sin(p1) - (alpha*Z2 - 1)*np.sin(alpha*p2)/alpha
        CRMef = abs(num/den)
...
def estXCR(psi, d, kCR, pCR, CRmax, a, F_L, Ra, Fa):
    ...
    # Vertex or minimum positive root
    xCRreg, CRreg = xCRVert(xCRIni, xCRSup, xCRMef,
                            CRIni, CRmax, CRMef, xCRInf)
    # Errors
...
def funDamp(mesh_Fin, TipLay, Lx, Ly, pml, nexp, eta_array,
            etacr, Z, fref, lmin, F_L, cref, psi=0.999, m=1):
    '''
...
def etafun(mesh_Fin, Lx, Ly, lref, pml, Z, fref,
           lmin, F_L, cref, TipLay='REC', nexp=np.nan):
    '''
    ...
    # Damping Function
    etalim = funDamp(mesh_Fin, TipLay, Lx, Ly, pml, nexp, eta_array,
                     etacr, Z, fref, lmin, F_L, cref)
