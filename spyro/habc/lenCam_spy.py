...
def calcZero(xini, a, nz=1):
    ...
    while abs(F(x, a)) > f_tol or f_tol > 1e-6:
        if (
            abs(F(x, a)) <= f_tol or F(x, a) * F(x - f_tol, a) < 0
        ) and x > xini + f_tol:
...
def calcp(x_rel, a, b, lim, pmax=20):
    ...
    h = max(
        int(
            np.ceil(
                np.log(0.5)
                / np.log((1 / a + 1 / b) / (1 / x_rel[0] + 1 / x_rel[1]))
            )
        ),
        2,
    )
    ...
    print(
        "x0(km):{:5.3f} - y0(km):{:5.3f}".format(x_rel[0] / 1e3, x_rel[1] / 1e3)
    )
    print("************************************")
    if lim == "MIN":
        if rs < r:
...
def CalcFL(TipLay, Lx, Ly, fref, lmin, lref, Z, nexp, nz=5, crtCR=0):
    ...
    if not TipLay == "rectangular":
        ...
        x_pml = [
            0.5 * Lx + pmlRect * np.cos(theta),
            0.5 * Ly + pmlRect * np.sin(theta),
        ]
        rf = abs(x_pml[0] / a) ** nexp + abs(x_pml[1] / b) ** nexp
        print("HypEll Limits: p:{}-r:{:3.4f}-rf:{:3.4f}".format(nexp, r, rf))
...
def habc_size(HABC):
    ...
    Lx = HABC.length_z
    Ly = HABC.length_x
    posCrit = HABC.neik_location
    f0 = HABC.frequency
    # it_fwi = HABC.fwi_iteration
    lmin = HABC.h_min
    # dt = HABC.dt
    Z = 1/HABC.neik_time_value
    nexp = np.nan
