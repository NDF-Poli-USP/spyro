...
def plot_receiver_difference(model, p_receiver0, p_receiver1, appear = False, name = 'Receivers', ft = 'PDF'):
    ...
    if times0 == times1:
        ...
        dt = final_time/times0
        p_receiver1 = spyro.tools.grid_point_calculator.time_interpolation(p_receiver1, p_receiver0, model)
    ...
    time_vector = np.linspace(0.0, final_time, dt)

    fig = plt.figure()
    ax = fig.add_subplot(111)
    plt.tick_params(reset=True, direction="in", which="both")
    plt.rc("legend", **{"fontsize": 18})
    plt.plot(time_vector, p_receiver0, 'bo', time_vector, p_receiver1, 'go' ) 
    ax.yaxis.get_offset_text().set_fontsize(18)
    plt.subplots_adjust(left=0.18, right=0.95, bottom=0.14, top=0.95)
    plt.legend(loc="best")
    plt.xlabel("time (s)", fontsize=18)
