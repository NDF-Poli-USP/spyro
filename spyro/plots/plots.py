...
def debug_pvd(function, filename="debug.pvd"):
    out = fire.VTKFile(filename)
    out.write(function)


def plot_model_in_p1(Wave_object, dx=0.01, filename="model.png",
                     abc_points=None, show=False, flip_axis=True):
    """
    Plots the velocity model of a given wave_object projected into
    a P1 (linear) finite element discretization. This function creates
    a deep copy of the input Wave_object's configuration, modifies
    it to use a P1 (degree 1) continuous Galerkin, and then generates
    a plot of the resulting velocity model.The plot can be saved to
    a file and optionally displayed.
    ...
    abc_points (list or None, optional):
        Points for absorbing boundary conditions to be marked on the plot.
        Defaults to None.
    show (bool, optional):
