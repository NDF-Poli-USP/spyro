# Adapted from pyadjoint/pyadjoint/checkpointing.py
# Original project: https://github.com/dolfin-adjoint/pyadjoint
# License: GNU LGPL v3
# Modifications: adapted for spyro, 2026
...
class RecomputeStrategy:
    """A base class for Recompute strategies used in checkpointing.
    """
    def Recompute_step(self, manager, step, current_step, cp_action, functional=None):
        raise NotImplementedError


class ExactRecompute(RecomputeStrategy):
    """_summary_

    Parameters
    ----------
    RecomputeStrategy : _type_
        _description_
    """
    def Recompute_step(self, manager, step, current_step, cp_action, functional=None):
...
class SpyroCheckpointManager:
    ...
    """
    def __init__(
            self, schedule, tape, gc_timestep_frequency=None, gc_generation=2,
        ...
        self.recompute_strategy = recompute_strategy or ExactRecompute()
    ...
    def end_timestep(self, timestep):
        ...
        while not self.process_first_fwd_computing(self._current_action, timestep + 1):
            self._current_action = next(self._schedule)
            self.forward_schedule.append(self._current_action)
    ...
    def process_first_fwd_computing(self, cp_action, timestep):
        """Implement checkpointing schedule actions for the first forward computation.
    ...
    @process_first_fwd_computing.register(Forward)
    ...
    @process_first_fwd_computing.register(EndForward)
    def _(self, cp_action, timestep):
