...
def generate_conditional_value_for_layer(
    domain_dim, mesh, dimension, ufl_coordinates_habc, type_marker="mask"
):
    ...
    z_pd = fire.conditional(condz, valz, 0.0)
    x_pd = fire.conditional(condx1, valx1, 0.0) + fire.conditional(condx2, valx2, 0.0)
    ...
    if dimension == 3:  # 3D
        ...
        # Conditional expressions for the mask
        y_pd = fire.conditional(condy1, valy1, 0.0) + fire.conditional(
            condy2, valy2, 0.0
        )
...
def layer_mask_field(
    ...
    if type_marker == "damping":  # Damping profile for the absorbing layer
        ...
        if bq == 0.0:
            ...
            ref_funct = fire.Constant(eta_crt) * (
                fire.Constant(aq) * ref_funct**2 + fire.Constant(bq) * ref_funct
            )

    elif type_marker == "mask":  # Mask filter for layer boundary domain

        ref_funct = fire.conditional(ref_funct > 0, 1.0, 0.0)
...
def clipping_coordinates_lay_field(
    domain_dim, mesh, dimension, ufl_coordinates_habc, V, quadrilateral=False
):
    ...
    lay_arr[:, 0] = clip(lay_arr[:, 0], -Lz, 0.0)
    lay_arr[:, 1] = clip(lay_arr[:, 1], 0.0, Lx)
    ...
    if dimension == 3:  # 3D
        ...
        # Clipping coordinates
        lay_arr[:, 2] = clip(lay_arr[:, 2], 0.0, Ly)

    # Mask function to identify the absorbing layer domain
    layer_mask = layer_mask_field(
        domain_dim, mesh, dimension, ufl_coordinates_habc, V, type_marker="mask"
    )
...
def extend_scalar_field_profile(
    mesh_original,
    field_to_extend,
    lay_field,
    layer_mask,
    ...
    if method == "point_cloud":
        ...
        vel_to_extend = point_cloud_field(
            mesh_original, pts_to_extend, field_to_extend, tolerance
        ).dat.data_with_halos[:]
