...
def layer_mask_field(domain_dim, mesh, dimension, ufl_coordinates_habc, V,
    ...
    validade_parameter('type_marker', type_marker, ["damping", "mask"])
    ...
    if type_marker == "damping":
        ...
        validade_numeric("pad_length", pad_length, float_num=True,
                              integer_num=True, lower_bound=0.)

        validade_numeric("eta_crt", eta_crt, float_num=True,
...
def extend_scalar_field_profile(mesh_original, field_to_extend, lay_field, layer_mask,
    ...
    validade_parameter('method', method, ["point_cloud", "nearest_point"])
