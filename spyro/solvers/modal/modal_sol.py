...
class Modal_Solver():
    ...
    c_equivalent()
        Compute an equivalent homogeneous velocity for an inhomogeneneous velocity model
        using an energy-equivalent homogenization.
    estimate_timestep()
        Estimate the maximum stable timestep based on the spectral radius using optionally
        the Gershgorin Circle Theorem to estimate the maximum generalized eigenvalue.
    ...
    freq_factor_hyp()
        Compute an approximate frequency factor for the hypershape with truncation plane
        at z = cut_plane_percent * b, b = Lz + pad_len.
    freq_factor_rec()
    ...
    def c_equivalent(self, c, V, quad_rule=None, typ_homog='energy',
        ...
        static_load_for_ceq : `Firedrake.Function`, optional
            ...
            Only used if 'typ_homog' is 'energy'. Default is `None`, in which
    ...
    def freq_factor_hyp(self, n_hyp, f_rec, f_ell, c_eq, bc="Neumann", c_eqref=None,
    ...
    def solver_analytical(self, c_eq, hyp_par, bc="Neumann", c_eqref=None,
        ...
        pprint(f"Hypershape Equivalent Velocity c_eq (km/s) = {c_reg:.3f}",
               comm=self.comm)
        pprint(f"Hypershape Frequency factor f_hyp (1/km): {f_hyp:.3f}",
               comm=self.comm)
