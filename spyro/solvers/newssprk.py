...
def SSPRKMOD(
    ...
    if PML:
        ...
        if dim == 2:
            W =  V* V * Z
            ddudt2_trial, dudt_trial, dppdt_trial = TrialFunctions(W)
            vw, v, qq = TestFunctions(W)
            sw, sv,sq = TestFunctions(W)
            dudt_n, u_n, pp_n = Function(W).split()
    ...
    outfile = helpers.create_output_file("SSPRKMOD_G"+str(G)+".pvd", comm, source_num)
    ...
    if PML:
        ...
        if dim == 2:
            ...
            FF += pml1 + pml2 + pml3
            # -------------------------------------------------------
            mm1 = (dot( dppdt_trial, qq) ) * dx(rule=qr_x)
