...
def _build_gradient_solver(
    wave_obj: Wave, mask_available: bool
    ...
    if wave_obj.abc_boundary_layer_type == "PML":
        ...
        ffG = -2 * (wave_obj.c) ** (-3) * fire.dot(forward_field, uadj) * m_v * dx
        parallel_print(
...
def _compute_dufordt2(forward_solution: list, dt: float) -> fire.Function:
    ...
    if len(forward_solution) > 2:
        return (
            forward_solution.pop() - 2.0 * forward_solution[-1] + forward_solution[-2]
        ) / fire.Constant(dt**2)
    else:
...
def _trapezoidal_gradient_integration(
    dJ: fire.Function, gradi: fire.Function, step: int, nt: int
) -> None:
