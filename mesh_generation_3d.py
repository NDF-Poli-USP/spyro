...
def generate_mesh3D(model, M, scale, comm):
    ...
    if comm.comm.rank == 0:
        ...
        meshio.write_points_cells("meshes/homogeneous_3D_scale"+str(scale)+".msh",
        points,[("tetra", cells)],
        file_format="gmsh22", 
        binary = False
        )
    meshio.write_points_cells("meshes/homogeneous_3D_scale"+str(scale)+".vtk",
        points,[("tetra", cells)],
        file_format="vtk"
        )
...
for scale in scales:
    ...
    comm.comm.barrier()
    if comm.ensemble_comm.rank == 0 and comm.comm.rank == 0:
            print("Meshing with M="+str(M), flush=True)
    comm.comm.barrier()
    generate_mesh3D(model, M, scale, comm)
