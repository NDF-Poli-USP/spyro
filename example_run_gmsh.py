...
def generate_gmsh_mesh(Lz, Lx, mesh_size, output_file="mesh.msh"):
    """Generate a rectangular mesh using gmsh with triangular elements.
    ...
    mesh_size : float
        Target edge length for mesh elements (m)
    output_file : str
    ...
    # Create rectangle geometry from (-Lz, 0) to (0, Lx)
    p1 = gmsh.model.geo.addPoint(-Lz, 0, 0, mesh_size)
    p2 = gmsh.model.geo.addPoint(0, 0, 0, mesh_size)
    p3 = gmsh.model.geo.addPoint(0, Lx, 0, mesh_size)
    p4 = gmsh.model.geo.addPoint(-Lz, Lx, 0, mesh_size)
...
def numerical_solution(j):
    ...
    mesh_file = generate_gmsh_mesh(L, L, h, output_file="elastic_mesh.msh")
