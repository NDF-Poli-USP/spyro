...
from firedrake import dx, Function
import numpy as np
...
def wave_dict(cell_type, domain_dimensions, final_time, dt):
    ...
    dictionary["acquisition"] = {
        "source_locations": [(-length_z / 2.0, length_x / 2.0, length_y / 2.0)],
        "frequency": 5.0,  # in Hz
        "receiver_locations": [
            (-length_z, 0.0, 0.0),
    ...
    for property_name, constant in zip(scalar_mat_prop, constant_lst):
        try:
            mat_property = wave.set_material_property(
                ...
                constant=constant,
                output=True,
                foldername="/property_fields/constant/",
            )
            ...
            dummy_vol = wave.set_material_property("dummy_vol", "scalar", constant=1.0)
            volume = fire.assemble(dummy_vol * dv)
            mean_val = fire.assemble(mat_property * dv) / volume

            assert np.isclose(
                mean_val, constant, rtol=1e-8
            ), f"{property_name}: Expected value {constant}, got {mean_val}"
            print(
                f"{property_name} Verified: expected "
                f"{constant}, got = {round(mean_val, 10)}",
                flush=True,
            )
    ...
    for property_name, random in zip(scalar_mat_prop, random_lst):
        try:
            mat_property = wave.set_material_property(
                ...
                random=random,
                output=True,
                foldername="/property_fields/random/",
            )
            ...
            assert np.all(
                mat_property_data >= min_val - 1e-8
            ), f"Values below minimum {min_val} for {property_name}"
            print(
                f"{property_name} Verified: Values " f">= minimum {min_val}", flush=True
            )
            assert np.all(
                mat_property_data <= max_val + 1e-8
            ), f"Values above maximum {max_val} for {property_name}"
            print(
                f"{property_name} Verified: Values " f"<= maximum {max_val}", flush=True
            )
...
def numerical_values_cond(property_name, coords, below_thrs, above_thrs):
    ...
    if property_name == "vel_P":
        exp_below = 2.0 + np.abs(z[below_thrs])
        exp_above = 1.5
    elif property_name == "vel_S":
        exp_below = (2.0 + np.abs(z[below_thrs])) / 2.5
        exp_above = 0.75
    elif property_name == "mass_rho":
        exp_below = 1.7e3 + 3e3 * np.abs(z[below_thrs]) ** 2
        exp_above = 1e3
    elif property_name == "epsilonTh":
        exp_below = np.exp(x[below_thrs]) / 10.0
        exp_above = 0.15 * np.exp(x[above_thrs])
    elif property_name == "gammaTh":
        exp_below = 2.5 * np.exp(x[below_thrs]) / 10.0
        exp_above = np.exp(x[above_thrs]) / 10.0
    elif property_name == "deltaTh":
        exp_below = -np.exp(x[below_thrs]) / 20.0
        exp_above = np.exp(x[above_thrs]) / 10.0
    elif property_name == "thetaTTI":
        exp_below = 1e4 * (y[below_thrs] - 0.08) ** 2 / 2.0 - 2.0
        exp_above = -(1e4 * (y[above_thrs] - 0.08) ** 2 / 2.0 - 2.0)
...
def test_conditional_mat_prop(wave_instance, cell_type):
    ...
    f_vel = 2.0 + np.abs(wave.mesh_z)
    cond_Vp = fire.conditional(wave.mesh_z < -0.06, f_vel, 1.5)
    cond_Vs = fire.conditional(wave.mesh_z < -0.06, f_vel / 2.5, 0.75)
    f_rho = 1.7e3 + 3e3 * np.abs(wave.mesh_z) ** 2
    cond_rho = fire.conditional(wave.mesh_z < -0.06, f_rho, 1e3)
    f_TH = fire.exp(wave.mesh_x) / 10.0
    cond_eps = fire.conditional(wave.mesh_x < 0.28, f_TH, 1.5 * f_TH)
    cond_gam = fire.conditional(wave.mesh_x < 0.28, 2.5 * f_TH, f_TH)
    cond_del = fire.conditional(wave.mesh_x < 0.28, -f_TH / 2.0, f_TH)
    f_TTI = 1e4 * (wave.mesh_y - 0.08) ** 2 / 2.0 - 2.0
    cond_the = fire.conditional(wave.mesh_y < 0.08, f_TTI, -f_TTI)
    cond_phi = fire.conditional(wave.mesh_y < 0.08, -f_TTI, f_TTI)
    cond_lst = [
        cond_Vp,
        cond_Vs,
        cond_rho,
    ...
    for property_name, cond_field in zip(scalar_mat_prop, cond_lst):
        try:
            mat_property = wave.set_material_property(
                property_name,
                "scalar",
                conditional=cond_field,
                output=True,
                foldername="/property_fields/conditional/",
            )
            ...
            print(
                f"{property_name} Verified: Conditional values "
                f"{coord_thrs} < {threshold}",
                flush=True,
            )
    ...
    for property_name, expr_field in zip(scalar_mat_prop, expr_lst):
        try:
            mat_property = wave.set_material_property(
                ...
                output=True,
                foldername="/property_fields/expression/",
            )
...
def get_coords_DG0(wave, coords):
    ...
    """
    coords_DG0 = coords[:]
    coords_DG0[:, 0] -= wave.mesh_parameters.edge_length / 2.0
    coords_DG0[:, 1] += wave.mesh_parameters.edge_length / 2.0
    coords_DG0[:, 2] += wave.mesh_parameters.edge_length / 2.0
    coords_DG0.round(2)
...
def test_fromfile_mat_prop(wave_instance, cell_type):
    ...
    dummy = wave.set_material_property("dummy", "scalar", constant=1.0)
    dummy.dat.data_with_halos[:] = vel_P.dat.data_with_halos[:] / 2.0

    from_file_segy = getcwd() + "/property_fields/from_file/vel_S.segy"
    create_segy(
        dummy,
        wave.function_space.sub(0),
        wave.mesh_parameters.edge_length,
        from_file_segy,
    )
    ...
    assert expect_msg in str(
        exc_info.value
    ), f"Unexpected error message: {str(exc_info.value)}"

    print(
        "Material Property from file: Correctly raised "
        f"NotImplementedError: {exc_info.value}",
        flush=True,
    )
...
def test_vector_mat_prop(wave_instance, cell_type):
    ...
    dummy = wave.set_material_property("dummy", "vector", constant=0.0)
    dummy.sub(0).assign(
        wave.set_material_property("dummy_z", "scalar", constant=alphaT_o)
    )
    dummy.sub(1).assign(
        wave.set_material_property("dummy_x", "scalar", constant=2 * alphaT_o)
    )
    dummy.sub(2).assign(
        wave.set_material_property("dummy_y", "scalar", constant=3 * alphaT_o)
    )
    ...
    exp_v0 = fire.assemble(dummy.sub(0) * dv) / volume
    val_v0 = fire.assemble(alphaT_dg0.sub(0) * dv) / volume
    exp_v1 = fire.assemble(dummy.sub(1) * dv) / volume
    val_v1 = fire.assemble(alphaT_dg0.sub(1) * dv) / volume
    exp_v2 = fire.assemble(dummy.sub(2) * dv) / volume
    val_v2 = fire.assemble(alphaT_dg0.sub(2) * dv) / volume
...
def test_tensor_mat_prop(wave_instance, cell_type):
    ...
    # Tensor 6x6
    dummy = wave.set_material_property(
        "dummy", "tensor", shape_func_space=shape_func_space, constant=0.0
    )
    ...
    # Same function space
    Celast = wave.set_material_property(
        "Celast", "tensor", fire_function=dummy, shape_func_space=shape_func_space
    )
    ...
    compC_val = fire.as_tensor(((C11, C33, C44), (C66, C12, C13)))
    ...
    for i in range(2):
        ...
        for j in range(3):
            ...
            row.append(
                wave.set_material_property(
                    compC_name[3 * i + j], "scalar", constant=val
                )
            )
        entries.append(row)
    tensor_expr = fire.as_tensor(entries)
