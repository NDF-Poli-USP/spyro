...
def test_random_mat_prop(wave_instance, cell_type):
    ...
    for property_name, random in zip(scalar_mat_prop, random_lst):
        try:
            ...
            print(
                f"{property_name} Verified: Values " f">= minimum {min_val}",
                flush=True,
            )
            assert np.all(
            ...
            print(
                f"{property_name} Verified: Values " f"<= maximum {max_val}",
                flush=True,
            )
...
def numerical_values_cond(property_name, coords, below_thrs, above_thrs):
    ...
    if property_name == "vel_P":
        exp_below = 2.0 + abs(z[below_thrs])
        exp_above = 1.5
    elif property_name == "vel_S":
        exp_below = (2.0 + abs(z[below_thrs])) / 2.5
        exp_above = 0.75
...
def get_only_mesh_vertices(wave_obj):
    ...
    """
    mesh_f = fire.Function(wave_obj.function_space).interpolate(
...
def test_conditional_mat_prop(wave_instance, cell_type):
    ...
    f_TH = fire.exp(wave_obj.mesh_x) / 10.0
    cond_eps = fire.conditional(wave_obj.mesh_x < 0.28, f_TH, 1.5 * f_TH)
    cond_gam = fire.conditional(wave_obj.mesh_x < 0.28, 2.5 * f_TH, f_TH)
    cond_del = fire.conditional(wave_obj.mesh_x < 0.28, -f_TH / 2.0, f_TH)
    f_TTI = 1e4 * (wave_obj.mesh_y - 0.08) ** 2 / 2.0 - 2.0
...
def get_coords_DG0(wave_obj, coords):
    ...
    """
    coords_DG0 = coords[:]
    coords_DG0[:, 0] -= wave_obj.mesh_parameters.edge_length / 2.0
    coords_DG0[:, 1] += wave_obj.mesh_parameters.edge_length / 2.0
    coords_DG0[:, 2] += wave_obj.mesh_parameters.edge_length / 2.0
    coords_DG0.round(2)
...
def test_fromfile_mat_prop(wave_instance, cell_type):
    ...
    dummy = wave_obj.set_material_property("dummy", "scalar", constant=1.0)
    dummy.dat.data_with_halos[:] = vel_P.dat.data_with_halos[:] / 2.0

    from_file_segy = getcwd() + "/property_fields/from_file/vel_S.segy"
    create_segy(
        dummy,
        wave_obj.function_space.sub(0),
        wave_obj.mesh_parameters.edge_length,
        from_file_segy,
    )
...
def test_vector_mat_prop(wave_instance, cell_type):
    ...
    dummy = wave_obj.set_material_property("dummy", "vector", constant=0.0)
    dummy.sub(0).assign(
...
def test_tensor_mat_prop(wave_instance, cell_type):
    ...
    # Tensor 2x3
    dummy = wave_obj.set_material_property(
        "dummy", "tensor", shape_func_space=(2, 3), constant=0.0
    )
