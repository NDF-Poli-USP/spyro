...
def run_tools(wave, method_extend, n_root=1):
    ...
    wave.layer_ops.velocity_abc(wave, method=method_extend)
    ...
    # Create layer mask
    method_element = "DQ" if wave.mesh_ops.quadrilateral else "DG"
    ...
    layer_mask = layer_mask_field(
        wave.mesh_ops.domain_dim, wave.mesh, wave.dimension,
        wave.mesh_ops.get_spatial_coordinates_abc(wave.mesh, domain_layer),
    ...
    # Cloud fields
    layer_cloud_xlt = point_cloud_field(wave.mesh, pts_layer_xlt, wave.velocity_model,
                                        wave.mesh_parameters.tol)

    layer_cloud_xge = point_cloud_field(wave.mesh, pts_layer_xge, wave.velocity_model,
                                        wave.mesh_parameters.tol)
    original_cloud_xlt = point_cloud_field(wave.mesh, pts_original_xlt, wave.velocity_model,
                                           wave.mesh_parameters.tol)
    original_cloud_xge = point_cloud_field(wave.mesh, pts_original_xge, wave.velocity_model,
                                           wave.mesh_parameters.tol)
    # Verify cloud values
...
def test_habc_tools(element_geometry, dimension):
    ...
    wave = preamble_tools(dictionary, edge_length, f_est, dimension)
