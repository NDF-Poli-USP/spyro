...
def wave_dict(element_type, dimension, degree_layer):
    ...
    dictionary["acquisition"] = {
        "source_type": "ricker",
        "source_locations": ([(-0.5, 0.25)] if dimension == 2 else [(-0.5, 0.25, 0.5)]),
        "frequency": 5.0,  # in Hz
        "delay": 1.5,
        "receiver_locations": (
            [(-Lz, 0.0), (-Lz, Lx), (0.0, 0.0), (0.0, Lx)]
            if dimension == 2
            else [
                (-Lz, 0.0, 0.0),
                (-Lz, Lx, 0.0),
...
def preamble_tools(dictionary, edge_length, f_est, dimension):
    ...
    wave_obj = habc.HABC_Wave(
        dictionary=dictionary, output_folder=f"output/test_habc_tools{dimension}d"
    )
...
def run_tools(wave_obj, method_extend, n_root=1):
    ...
    layer_mask = layer_mask_field(
        wave_obj.mesh_ops.domain_dim,
        wave_obj.mesh,
        wave_obj.dimension,
        wave_obj.get_spatial_coordinates_habc(),
        V,
        type_marker="mask",
        name_mask="test_mask",
    )
    ...
    mask_nodes = wave_obj.mesh_ops.extract_node_positions(
        wave_obj.mesh, V, output_type="array"
    )
    indlay_nodes = np.where(layer_mask.dat.data_with_halos == 1.0)[0]
    ...
    original_nodes = np.where(layer_mask.dat.data_with_halos == 0.0)[0]
    ...
    # Cloud fields
    layer_cloud_xlt = point_cloud_field(
        wave_obj.mesh, pts_layer_xlt, wave_obj.c, wave_obj.mesh_parameters.tol
    )

    layer_cloud_xge = point_cloud_field(
        wave_obj.mesh, pts_layer_xge, wave_obj.c, wave_obj.mesh_parameters.tol
    )
    original_cloud_xlt = point_cloud_field(
        wave_obj.mesh, pts_original_xlt, wave_obj.c, wave_obj.mesh_parameters.tol
    )
    original_cloud_xge = point_cloud_field(
        wave_obj.mesh, pts_original_xge, wave_obj.c, wave_obj.mesh_parameters.tol
    )
    # Verify cloud values
    met_str = (
        f"HABC Tools {ele_str}-{ext_str}"
        + f" {wave_obj.case_habc[:-4]} {wave_obj.dimension}D. "
    )
    expected_values = [expect_xmhalf, expect_xphalf, expect_xmhalf, expect_xphalf]
    mean_val = [
        layer_cloud_xlt.dat.data_with_halos.mean(),
    ...
    for region, exp_value, mean_val in zip(region_names, expected_values, mean_val):
        ...
        assert np.isclose(mean_val / exp_value, 1.0, atol=tolerance), (
            "✗ " + met_str + "  → " + cmp_str
        )
...
def test_habc_tools(element_type, dimension, method_extend):
    ...
    degree_layer = 3.0 if element_type == "T" else None
