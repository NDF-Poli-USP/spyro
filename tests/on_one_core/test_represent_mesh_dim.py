...
def test_representative_mesh_dimensions(element_type, dimension):
    ...
    mesh_derived_parameters = Wave_obj.mesh_ops.representative_mesh_dimensions(
    ...
    assert (
        Wave_obj.mesh_parameters.lmin > 0
    ), f"lmin should be positive, got {Wave_obj.mesh_parameters.lmin}"
    assert (
        Wave_obj.mesh_parameters.lmax > 0
...
def test_boundary_ids(element_type, dimension):
    ...
    domain_dim = (1.0, 1.0) if dimension == 2 else (1.0, 1.0, 1.0)
    quadrilateral = True if element_type == "Q" else False
    mesh_ops = mshops.MeshOps(
        domain_dim,
        dimension=dimension,
    ...
    if quadrilateral:
        ...
        element = fire.FiniteElement(
            family, mesh.ufl_cell(), degree=degree_ele, variant="spectral"
        )
        V = fire.FunctionSpace(mesh, element)
    else:
        mesh = (
            fire.UnitSquareMesh(10, 10)
            if dimension == 2
    ...
    boundary_idx_map = mesh_ops.mapping_boundary_ids(
        mesh, V, boundaries, box_domain=True
    )
    ...
    print(
        f"✓ Boundary ID mapping for {element_type} {dimension}D mesh is correct.",
        flush=True,
    )
