...
def test_gmsh2d():
    marmousi_params = {
        ...
        "length_z": 3500.0,   
        "length_x": 17000.0,
        "output_filename": "marmousi.msh",
        
        # SEGY / Seismic constraints
        "velocity_model": "MODEL_S-WAVE_VELOCITY_1.25m.segy", # Velocity model segy file
        "cells_per_wavelength": 2.0, 
        "source_frequency": 3.0,
        
        # Padding Parameters
        "padding_type": "rectangular" , # Padding types "rectangular" "hyperelliptical" None 
        "padding_x": 1500.0, # Padding size in x direction
        "padding_z": 1500.0, # Padding size in z direction
        "hyper_n": 3.0, # Hyperellipse exponent
        "hmin_segy": 0.0, # Minimum Element size for segy, will apply if higher than function minimum
        "grade": 0.9, # function grading for smooth element transition, None = no smooth, 0.9 = small smooth, 0.1 = high smooth
        ...
        "min_element_size": 50.0, # Element size for structured mesh
    ...
    mesh_params = MeshingParameters(input_mesh_dictionary=marmousi_params, velocity_model=marmousi_params["velocity_model"])
