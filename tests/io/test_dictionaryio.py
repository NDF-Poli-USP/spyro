...
def test_read_options_all_fields():
    ...
    assert options.variant == Variant.LUMPED
    assert options.method == Method.MASS_LUMPED_TRIANGLE
    assert options.cell_type == CellType.TRIANGLE
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2, 2, Variant.LUMPED, Method.SPECTRAL_QUADRILATERAL, "invalid"), # invalid cell type   
        (2, 2, None, Method.CG, None), # invalid, should specify both variant and cell_type
        (2, 2, None, Method.MASS_LUMPED_TRIANGLE, CellType.QUADRILATERAL), # invalid, cell_type should be TRIANGLE
    ],
)
