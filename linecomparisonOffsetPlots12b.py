...
#     ax[cont].plot(xs, error, "-")
#     ax[cont].plot([2.0,10.0],[5, 5], 'k--')
#     max_error_location = xs[np.argmax(error)][0]
#     max_error = np.amax(error)
#     ax[cont].plot([max_error_location ,max_error_location , 2.0 ],[0.0  , max_error  , max_error ], '--')
#     print("Max error location is "+str(max_error_location)+ " km.")
#     print("Max error value is "+ str(max_error)+" %")

#     ax[cont].set_xlim((2.0,10.0))
#     ax[cont].set_ylim((0.0,9.0))
...
#     ax[cont].set_xticks(xticks)
#     ax[cont].set_yticks(yticks)
#     ax[cont].xaxis.set_major_formatter(FormatStrFormatter('%.2f'))
#     ax[cont].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
#     ax[cont].set(xlabel = "offset (km)", ylabel = "$r_{error}$ %")
#     if p20more:
#         withoffset = ' with 20% more G'
#     else:
#         withoffset = ' without 20% more G'
#     ax[cont].set_title("Error with varying offset for KMV"+str(degree)+"tri on BP2004 velocity model")
#     cont += 1
#     #plt.yscale("log")
#     #plt.show()
...
time_vector0 = np.linspace(0.0, final_time, nt0)
colors = ['b', 'y', 'g', 'r']
#ax.plot(time_vector0, p_receiver0[:, id], label = 'Reference', c = 'b', linewidth = 0.5 ) 
...
for degree in degrees:
    ...
    #plt.rc("legend")
    #fig, ax = plt.subplots()
    diff = (p_receiver1[:, id]- p_receiver0[:,id])
    
    ax.plot(time_vector1, diff, label = 'KMV'+str(degree)+'tri', c = colors[cont],linewidth = 0.5 ) 
    ax.legend(loc="best")
    ## Cutting of to zoom in at time interval start_time-stop_time
    ax.set_xlim((start_time,stop_time))

    #ax2.subplots_adjust(left=0.18, right=0.95, bottom=0.14, top=0.95)
    ax.set(xlabel = "simulated time (s)", ylabel = "difference in pressure")
    #ax.set_title("Pressure at receiver on "+str(np.round(max_error_location,2))+ " km offset")
    ax.set_title("Error at receiver with 2 km offset")
    #plt.xlim(model['acquisition']['delay']*0.8, final_time)
    #plt.ylim(tf, 0)
    #plt.savefig("Receivers." + ft, format=ft)
    # plt.axis("image")
