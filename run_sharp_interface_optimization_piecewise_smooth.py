...
def update_velocity(mesh, V, q, vp):
    ...
    # create piecewise smooth velocity field
    Z, _ = SpatialCoordinate(mesh)
    vp_new = Function(V, name="velocity").interpolate(Min(1.5 + 4.0 * abs(Z), 4.1))
    ...
    for sn in range(model["acquisition"]["num_sources"]):
        if spyro.io.is_owner(comm, sn):
            ...
            if comm.comm.rank == 0:
                ...
                plt.plot(p_exact_recv[:-1:1, 50], "k-")
...
def optimization(model, mesh, V, comm, vp, sources, receivers, max_iter=10):
    ...
    while iter_num < max_iter:
        ...
        vp_new = update_velocity(mesh, V, indicator_new, vp)
