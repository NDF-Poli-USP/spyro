...
if (vel_model == "marmousi" or vel_model == "br_model"):
    # vp_guess = spyro.io.interpolate(model["mesh"]["initmodel"], model, mesh, V)  
    # np.save("mm_guess.npy", vp_guess.dat.data[:])
    # quit()
...
def runfwi(solver_type, tot_source_num, comm, xi, sn=0):
    ...
    if aut_dif:
        ...
        dJ = comp_grad(Jm, control)
                    #    l, "riesz_representation" == "L2")

        fire_adj.get_working_tape().clear_tape()

    else:
        solver_ad.solver = "fwd"
        out = wp(comm, vp_guess, sources, receivers, wavelet, 
                 calc_functional=True,  true_rec=p_exact_recv,
                 save_misfit=True, save_p=True)
        ...
        start = tm.time()
        out = wp(comm, vp_guess, sources, receivers,
                 wavelet, misfit=misfit, p_guess=p_guess)
        end = tm.time()
        print(end-start)
        dJ = out[0]
    return Jm, dJ
