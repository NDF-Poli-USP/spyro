...
if platform.node()=='recruta':
    path = ""
    #sys.exit("path not defined") FIXME uncomment it
else:
    path = "/share/tdsantos/shots/elastic_forward_camembert_15Hz_4_sources/" 
...
if AMR==1 and GUESS==1:
    ...
    if DG_VP==1:
        element_DG = spyro.domains.space.FE_method(mesh, "DG", 0) #FIXME model["opts"]["degree"])
        V_DG = FunctionSpace(mesh, element_DG)
        _, _vs, _ = _make_vs(V_DG)
    else: 
        _, _vs, _ = _make_vs(V) # V is the original space of mesh
