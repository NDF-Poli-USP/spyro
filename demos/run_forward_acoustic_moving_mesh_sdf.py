# run_forward_acoustic_moving_mesh_sdf.py
...
model["acquisition"] = {
    ...
    #FIXME "source_pos": spyro.create_transect((0.1, 0.9), (0.9, 0.9), 4), # waterbottom at z=-0.45 km # out of domain, only to run with some source
    "source_pos": spyro.create_transect((0.1, 0.9), (0.9, 0.9), 1), # waterbottom at z=-0.45 km # out of domain, only to run with some source
...
#}}}
# make vp background layers {{{
def _make_vp_background_layers(V):
    ...
    vp_cond = conditional(y >= 0.7, vu-y+1, v0 - 2.5*(y-0.7))
    vp_cond = conditional(y <= 0.5 - 0.2*x, vl-0.5*y+1.0, vp_cond)

    return v0, dv, vp_cond
#}}}
# make vp circle {{{
def _make_vp_circle(V, xc, yc, r):
    ...
    v0, dv, vp_cond = _make_vp_background_layers(V)
    vp_cond = conditional((x-xc)**2 + (y-yc)**2 <= r**2, v0+dv, vp_cond)
    vp = Function(V).interpolate(vp_cond)

    File("exact_vp_circle.pvd").write(vp)

    return vp
#}}}
# make vp rectangle {{{
def _make_vp_rectangle(V, x1, x2, y1, y2):
    ...
    v0, dv, vp_cond = _make_vp_background_layers(V)
    vp_cond = conditional( And(And(x>x1,y>y1),And(x<x2,y<y2)), v0+dv, vp_cond)
    vp = Function(V).interpolate(vp_cond)

    File("exact_vp_rectangle.pvd").write(vp)

    return vp
#}}}
# make sdf circle {{{
def _make_sdf_circle(V, xc, yc, r):
    ...
    sdf = Function(V).interpolate( ( (x-xc)**2 + (y-yc)**2 )**0.5 - r)
    
    File("sdf_circle.pvd").write(sdf)

    return sdf
#}}}
# make sdf rectangle {{{
def _make_sdf_rectangle(V,x1,x2,y1,y2):
    ...
    sdf = Function(V).interpolate( -min_value(min_value(min_value(-y1+y,y2-y),-x1+x),x2-x) )

    File("sdf_rectangle.pvd").write(sdf)

    return sdf
#}}}}
# make vp using vp_type {{{
def _make_vp_sdf(V, vp_type, vec_data):
    ...
    if vp_type==1: # circle
        ...
        vp = _make_vp_circle(V, xc, yc, r)
        sdf = _make_sdf_circle(V, xc, yc, r)
    
    elif vp_type==2: # rectangle
        x1 = vec_data[0]
        x2 = vec_data[1]
        y1 = vec_data[2]
        y2 = vec_data[3]
        
        vp = _make_vp_rectangle(V, x1, x2, y1, y2)
        sdf = _make_sdf_rectangle(V, x1, x2, y1, y2)
...
vp_type = 2 # 1 = circle, 2 = rectangle
# for circle
_xc = 0.5
_yc = 0.5
_r = 0.25
...
if platform.node()=='recruta':
    ...
    #FIXME change here the path according to the model setup 
    path = "/share/tdsantos/shots/acoustic_forward_sdf_15Hz_4_sources/" 

# run reference model {{{
if REF:
    _nx = 100  # nx=100  => dx = dz = 10 m
    #_nx = 20  # FIXME nx=100  => dx = dz = 10 m
    ...
    if DG_VP==1:# DG space is better to represent data, but it is not currently possible to use it during FWI
        ...
        #vp_ref = _make_vp_circle(V_ref_DG) 
        vp_ref, _ = _make_vp_sdf(V_ref_DG, vp_type, vec_data) 
    else:
        #vp_ref = _make_vp_circle(V_ref)
        vp_ref, _ = _make_vp_sdf(V_ref, vp_type, vec_data) 
        #sdf = _make_sdf_circle(V_ref)
        #x1 = 0.3
        #x2 = 0.7
        #y1 = 0.3
        #y2 = 0.7
        #sdf = _make_sdf_rectangle(V_ref,x1,x2,y1,y2)
        #sys.exit("exit")
    ...
    #FIXME p_ref_recv = spyro.io.load_shots(model, comm, file_name=path+file_name)
#}}}
...
if AMR==1 and GUESS==1:
    ...
    vp_grid, sdf_grid = _make_vp_sdf(V_grid, vp_type, vec_data) 
    
    # Huang type monitor function
    E1 = vp_grid.vector().gather().max() / vp_grid - 1 # a priori error estimate (it starts on 1, so it is better)
    n = 2
    eps = 1.e-5
    E2 = 1 / (sdf_grid**n + eps)# sdf based estimate
    ...
    beta = 0.2 # (0, 1) # for E2 w/n smooth
    phi = sqrt( 1 + E*E ) - 1
    phi_hat = assemble(phi*dx(domain=mesh_grid)) / assemble(Constant(1.0)*dx(domain=mesh_grid))
    alpha = beta / ( phi_hat * ( 1 - beta ) )
    M2 = 1 + alpha * phi
    ...
    if DG_VP==1:
        ...
        _vp, _ = _make_vp_sdf(V_DG, vp_type, vec_data)
    else: 
        _vp, _ = _make_vp_sdf(V, vp_type, vec_data)
