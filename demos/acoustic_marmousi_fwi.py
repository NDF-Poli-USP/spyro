"""Demo script for running a marmousi FWI case.

This case is with mesh generation. It runs 16 shots for the marmousi FWI,
using the shot record created from a different mesh and using the 'true' velocity model.
...
You can experiment with a different number of cores if desired, or, if you want a different number of cores,
you can check out our different custom parallelism options.
"""
...
def run_fwi():
    ...
    fwi_obj.set_guess_velocity_model(
        new_file="initial_guess.hdf5", fast_interpolate=True
    )
    fwi_obj.run_fwi(
        vmin=1.4,
        vmax=4.7,
